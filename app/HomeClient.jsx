...
export default function HomeClient({ initialListings, initialTotal, initialStats, initialCounties, initialFilter, initialPage }) {
  ...
  // ✅ FIX: Only parse URL on client side
  useEffect(() => {
    if(typeof window === 'undefined') return;
    parseAndApplyURL(window.location.pathname, window.location.search);
  }, [parseAndApplyURL]);

  // ✅ FIX: Only add event listeners on client side
  useEffect(() => {
    if(typeof window === 'undefined') return;
  ...
  // ✅ FIX: Only access window in useEffect
  ...
  // ✅ FIX: Handle Google OAuth callback + password reset token - only on client
