...
export default function HomeClient({ initialListings, initialTotal, initialStats, initialCounties, initialFilter, initialPage }) {
  ...
  // ── HANDLE URL TOKENS FIRST — before URL router runs ─────────────────────
  useEffect(()=>{
    ...
    // Handle password reset token — show modal immediately
    if(rt){
      setResetToken(rt);
      window.history.replaceState({},"","/");
      return;
    ...
    if(vt){
      ...
      }).catch(e=>notify(e.message||"Verification failed. The link may have expired.","error"));
      window.history.replaceState({},"","/");
      return;
    }
  ...
  const parseAndApplyURL = useCallback((path, search) => {
    ...
    // Skip if there are special tokens in the URL — handled above
    if(params.get('reset_token')||params.get('verify_email')||params.get('auth_token')) return;
    ...
    if (section === 'sold') {
      ...
      setPage('dashboard');
      if (sub) window.__initialDashTab = sub;
  ...
  useEffect(()=>{
    ...
    s.on("notification",(n)=>{
      setNotifCount(c=>c+1);
      if(["listing_match","request_match","listing_approved","seller_pitch","pitch_accepted"].includes(n.type)){
      ...
      if(n.type==="warning"||n.type==="suspension"){
        ...
        apiCall("/api/auth/me",{},token).then(fresh=>{
          ...
          if(fresh.is_suspended){
            setTimeout(()=>{localStorage.removeItem("ws_token");localStorage.removeItem("ws_user");setUser(null);setToken(null);notify("Account suspended. Contact support@wekasoko.co.ke","error");},3000);
          }
        }).catch(()=>{});
      } else {
    ...
    s.on("new_message_inbox",()=>setNotifCount(c=>c+1));
    return()=>s.disconnect();
  },[token,user,notify]);
  ...
  useEffect(()=>{
    ...
    const subscribe=async()=>{
      try{
        ...
        if(existing){await apiCall("/api/push/subscribe",{method:"POST",body:JSON.stringify({subscription:existing})},token).catch(()=>{});return;}
  ...
  const sharedModals=<>
    ...
    {modal?.type==="detail"&&<DetailModal
      ...
      onChat={()=>{if(!user){setModal({type:"auth",mode:"login"});return;}setModal({type:"chat",listing:modal.listing});}}
      onLockIn={()=>handleLockIn(modal.listing)}
      onUnlock={()=>setModal({type:"pay",payType:"unlock",listing:modal.listing})}
      onEscrow={()=>{if(!user){setModal({type:"auth",mode:"login"});return;}setModal({type:"pay",payType:"escrow",listing:modal.listing});}}
    />}
    ...
    {modal?.type==="pay"&&user&&<PayModal
      type={modal.payType} listingId={modal.listing.id}
      ...
      onSuccess={async(result)=>{
        const ul=result.listing||(await apiCall(`/api/listings/${modal.listing.id}`,{},token).catch(()=>null));
        if(ul){setListings(p=>p.map(l=>l.id===ul.id?ul:l));closeModal();setTimeout(()=>setModal({type:"detail",listing:ul}),200);}else{closeModal();}
  ...
  return <>
      ...
      <div style={{display:"flex",alignItems:"stretch",minHeight:460,flexWrap:"wrap"}}>
        ...
        <div style={{flex:"1 1 280px",display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:1,minWidth:280}}>
          {CATS.slice(0,8).map((c)=>(
            <div key={c} onClick={()=>{setFilter(f=>({...f,cat:c}));setPg(1);}} style={{background:CAT_PHOTOS[c]?`url(${CAT_PHOTOS[c]}) center/cover`:"#E0E0E0",cursor:"pointer",display:"flex",alignItems:"flex-end",padding:"16px",minHeight:110,position:"relative",overflow:"hidden"}}>
        ...
        <div style={{marginBottom:32}}>
          <div style={{display:"flex",gap:12,flexWrap:"wrap",marginBottom:16}}>
            ...
            <div style={{display:"flex",gap:2}}>
              <button onClick={()=>setVm("grid")} style={{background:vm==="grid"?"#1428A0":"#fff",color:vm==="grid"?"#fff":"#767676",border:"1.5px solid #E0E0E0",padding:"8px 14px",cursor:"pointer",fontSize:15,fontFamily:"var(--fn)",borderRadius:"8px 0 0 8px"}}>⊞</button>
              <button onClick={()=>setVm("list")} style={{background:vm==="list"?"#1428A0":"#fff",color:vm==="list"?"#fff":"#767676",border:"1.5px solid #E0E0E0",borderLeft:"none",padding:"8px 14px",cursor:"pointer",fontSize:15,fontFamily:"var(--fn)",borderRadius:"0 8px 8px 0"}}>☰</button>
            </div>
            {user&&<button className="btn bp" style={{borderRadius:9,fontSize:14,padding:"9px 20px"}} onClick={()=>{if(user.role==="buyer"){if(window.confirm("Switch to Seller to post ads?"))apiCall("/api/auth/role",{method:"PATCH",body:JSON.stringify({role:"seller"})},token).then(d=>{const upd={...user,...d.user};setUser(upd);localStorage.setItem("ws_user",JSON.stringify(upd));notify("Switched to Seller!","success");setModal({type:"post"});}).catch(e=>notify(e.message,"error"));return;}setModal({type:"post"});}}>+ Post Ad</button>}
          </div>
        </div>
    ...
    <div style={{background:"#1428A0",padding:"clamp(28px,4vw,40px) clamp(20px,4vw,48px)",marginBottom:64,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:0,textAlign:"center"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:20}}>
        {[["📝","Post for Free","No upfront cost. Photos, description, location — done in 2 minutes."],
          ...
          <div key={title} style={{background:"#F4F4F4",padding:"28px 24px"}}>
            ...
            <div style={{fontWeight:700,fontSize:16,marginBottom:10,color:"#1A1A1A"}}>{title}</div>
    ...
    {page==="sold"&&<div style={{minHeight:"100vh",background:"#F0F0F0"}}>
      <div style={{background:"#1D1D1D",padding:"clamp(28px,4vw,52px) clamp(16px,4vw,40px) clamp(28px,4vw,48px)"}}>
        <button onClick={()=>setPage("home")} style={{background:"transparent",border:"1px solid rgba(255,255,255,.35)",color:"#fff",padding:"7px 16px",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"var(--fn)",marginBottom:28,display:"inline-flex",alignItems:"center",gap:6,borderRadius:8}}>← Back to Marketplace</button>
        <div style={{marginBottom:14,opacity:.9}}><WekaSokoLogo size={26}/></div>
        <h1 style={{fontSize:"clamp(30px,5vw,54px)",fontWeight:700,color:"#fff",lineHeight:1.05,marginBottom:14}}>Sold on Weka Soko</h1>
      ...
      <div style={{padding:"44px 48px 80px"}}><SoldSection token={token} user={user}/></div>
    </div>}
    ...
    {page==="dashboard"&&user&&<Dashboard user={user} token={token} notify={notify}
      onPostAd={()=>{setPage("home");window.history.pushState({},"","/");setModal({type:"post"});}}
      onClose={()=>{setPage("home");window.history.pushState({},"","/");}}
      onUserUpdate={updated=>{const m={...user,...updated};setUser(m);localStorage.setItem("ws_user",JSON.stringify(m));}}
      initialTab={window.location.pathname.startsWith("/dashboard/")?window.location.pathname.split("/dashboard/")[1]:undefined}
    />}
