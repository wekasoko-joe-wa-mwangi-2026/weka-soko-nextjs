...
function Pager({ total, perPage, page, onChange }) {
  ...
  const btn = (n, label, disabled) => (
    <button key={label||n} onClick={() => !disabled && onChange(n)} disabled={disabled}
      style={{padding:'6px 10px',border:'1px solid '+(n===page&&!label?'#2563eb':'#e5e7eb'),borderRadius:6,
        cursor:disabled?'default':'pointer',background:n===page&&!label?'#2563eb':'white',
        color:n===page&&!label?'white':disabled?'#bbb':'#374151',fontWeight:n===page&&!label?600:400,minWidth:36}}>
      {label || n}
    </button>
  );
  return (
    <div style={{display:'flex',gap:4,justifyContent:'center',alignItems:'center',padding:'16px 0',flexWrap:'wrap'}}>
      {btn(page - 1, '‹', page <= 1)}
      {s > 1 && <>{btn(1)}{s > 2 && <span style={{color:'#9ca3af',padding:'0 4px'}}>…</span>}</>}
      {nums.map(n => btn(n))}
      {e < pages && <>{e < pages - 1 && <span style={{color:'#9ca3af',padding:'0 4px'}}>…</span>}{btn(pages)}</>}
...
// ── PWA BANNER ───────────────────────────────────────────────────────────────
function PWABanner({ onDismiss }) {
  const [deferredPrompt, setDeferredPrompt] = React.useState ? undefined : null;
  ...
  return (
    <div style={{position:'fixed',bottom:16,left:'50%',transform:'translateX(-50%)',
      background:'#1e293b',color:'white',borderRadius:12,padding:'14px 20px',
      display:'flex',alignItems:'center',gap:12,zIndex:9999,boxShadow:'0 8px 30px rgba(0,0,0,.3)',
      maxWidth:360,width:'calc(100% - 32px)'}}>
      <span style={{fontSize:28}}>📱</span>
      <div style={{flex:1}}>
        <div style={{fontWeight:600,fontSize:14}}>Add Weka Soko to Home Screen</div>
        <div style={{fontSize:12,color:'#94a3b8',marginTop:2}}>Get faster access, offline support & notifications</div>
      </div>
      <div style={{display:'flex',flexDirection:'column',gap:6}}>
        {dp && <button onClick={install} style={{background:'#2563eb',color:'white',border:'none',borderRadius:6,padding:'6px 12px',cursor:'pointer',fontSize:12,fontWeight:600}}>Install</button>}
        <button onClick={onDismiss} style={{background:'transparent',color:'#94a3b8',border:'none',cursor:'pointer',fontSize:12}}>Dismiss</button>
      </div>
    </div>
  );
}
...
// ── STAR PICKER ──────────────────────────────────────────────────────────────
function StarPicker({ value = 0, onChange, readOnly = false }) {
  const [hover, setHover] = useState(0);
  return (
      ...
      {[1,2,3,4,5].map(n => (
        <span key={n}
          ...
          style={{fontSize:24,cursor:readOnly?'default':'pointer',
            color:(hover||value)>=n?'#f59e0b':'#d1d5db',transition:'color .15s'}}>★</span>
      ))}
    </div>
  );
}
...
// ── SOLD SECTION ─────────────────────────────────────────────────────────────
function SoldSection({ token, user, initialItems }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api('/listings?status=sold&page=' + pg + '&perPage=' + PG, {}, token)
  ...
  if (!items.length) return (
    <div style={{textAlign:'center',padding:60,color:'#6b7280'}}>
      <div style={{fontSize:48,marginBottom:12}}>🎉</div>
      <div style={{fontWeight:600,fontSize:18,marginBottom:8}}>No sold listings yet</div>
      <div>Items marked as sold will appear here.</div>
    </div>
  );
  return (
    <div>
      <h2 style={{fontWeight:700,fontSize:22,marginBottom:20}}>Sold Listings</h2>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(240px,1fr))',gap:16}}>
        {items.map(l => (
          <div key={l.id} style={{background:'white',borderRadius:12,overflow:'hidden',boxShadow:'0 1px 4px rgba(0,0,0,.08)',opacity:.8}}>
            {l.photos?.[0] && <img src={l.photos[0]} alt={l.title} style={{width:'100%',height:180,objectFit:'cover'}}/>}
            <div style={{padding:12}}>
              <div style={{fontWeight:600,marginBottom:4,fontSize:15}}>{l.title}</div>
              <div style={{color:'#2563eb',fontWeight:700}}>{fmtKES(l.price)}</div>
              <div style={{fontSize:12,color:'#6b7280',marginTop:4}}>{l.county} · {ago(l.updated_at)}</div>
              <div style={{marginTop:8,background:'#dcfce7',color:'#166534',borderRadius:6,padding:'4px 10px',fontSize:12,fontWeight:600,display:'inline-block'}}>✓ Sold</div>
            </div>
          </div>
        ))}
      </div>
      <Pager total={total} perPage={PG} page={pg} onChange={setPg} />
    </div>
  );
}

// ── REVIEWS SECTION ──────────────────────────────────────────────────────────
function ReviewsSection({ token, userId, isSelf }) {
  const [reviews, setReviews] = useState([]);
  const [loading, setLoading] = useState(true);
  const [stats, setStats] = useState(null);
  useEffect(() => {
    if (!userId) return;
    api('/reviews?userId=' + userId, {}, token)
      .then(d => { setReviews(d.reviews || d || []); setStats(d.stats || null); })
      .catch(() => {})
      .finally(() => setLoading(false));
  }, [userId, token]);
  ...
  return (
    <div>
      {stats && (
        <div style={{background:'#f8fafc',borderRadius:10,padding:16,marginBottom:20,display:'flex',gap:16,alignItems:'center'}}>
          <div style={{textAlign:'center'}}>
            <div style={{fontSize:36,fontWeight:700,color:'#1e293b'}}>{(stats.avg||0).toFixed(1)}</div>
            <StarPicker value={Math.round(stats.avg||0)} readOnly />
            <div style={{fontSize:12,color:'#6b7280'}}>{stats.count||0} review{stats.count!==1?'s':''}</div>
          </div>
          <div style={{flex:1}}>
            {[5,4,3,2,1].map(n => {
              ...
              return (
                <div key={n} style={{display:'flex',alignItems:'center',gap:8,marginBottom:4}}>
                  <span style={{fontSize:12,color:'#6b7280',minWidth:8}}>{n}</span>
                  <span style={{color:'#f59e0b',fontSize:12}}>★</span>
                  <div style={{flex:1,height:6,background:'#e5e7eb',borderRadius:3}}>
                    <div style={{height:'100%',background:'#f59e0b',borderRadius:3,width:pct+'%'}}/>
                  </div>
                  <span style={{fontSize:12,color:'#6b7280',minWidth:20}}>{c}</span>
                </div>
              );
            })}
          </div>
      ...
      {!reviews.length && <div style={{textAlign:'center',padding:40,color:'#6b7280'}}>No reviews yet.</div>}
      <div style={{display:'flex',flexDirection:'column',gap:12}}>
        {reviews.map(r => (
          <div key={r.id} style={{background:'white',borderRadius:10,padding:16,boxShadow:'0 1px 3px rgba(0,0,0,.08)'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8}}>
              <div>
                <div style={{fontWeight:600,fontSize:14}}>{r.reviewer_name || 'Anonymous'}</div>
                <StarPicker value={r.rating} readOnly />
              </div>
              <div style={{fontSize:12,color:'#9ca3af'}}>{ago(r.created_at)}</div>
            </div>
            {r.comment && <p style={{margin:0,color:'#374151',fontSize:14,lineHeight:1.5}}>{r.comment}</p>}
          </div>
        ))}
      </div>
    </div>
...
// ── MY REQUESTS TAB ──────────────────────────────────────────────────────────
function MyRequestsTab({ token, user, notify }) {
  const [requests, setRequests] = useState([]);
  const [loading, setLoading] = useState(true);
  const [deleting, setDeleting] = useState(null);
  const load = () => {
    setLoading(true);
    api('/requests?mine=1', {}, token)
  ...
  if (!requests.length) return (
    <div style={{textAlign:'center',padding:40,color:'#6b7280'}}>
      ...
      <div style={{fontWeight:600,marginBottom:6}}>No requests yet</div>
      <div style={{fontSize:14}}>Post what you're looking for and sellers will pitch to you.</div>
    </div>
  );
  return (
    <div style={{display:'flex',flexDirection:'column',gap:12}}>
      {requests.map(r => (
        <div key={r.id} style={{background:'white',borderRadius:10,padding:16,boxShadow:'0 1px 3px rgba(0,0,0,.07)'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:12}}>
            <div style={{flex:1}}>
              <div style={{fontWeight:600,fontSize:15,marginBottom:4}}>{r.title}</div>
              <div style={{fontSize:13,color:'#6b7280',marginBottom:6}}>{r.description?.slice(0,100)}{r.description?.length>100?'…':''}</div>
              <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                {r.budget && <span style={{background:'#dbeafe',color:'#1e40af',borderRadius:20,padding:'2px 10px',fontSize:12}}>Budget: {fmtKES(r.budget)}</span>}
                {r.county && <span style={{background:'#f3f4f6',color:'#374151',borderRadius:20,padding:'2px 10px',fontSize:12}}>{r.county}</span>}
                <span style={{background:'#f3f4f6',color:'#6b7280',borderRadius:20,padding:'2px 10px',fontSize:12}}>{ago(r.created_at)}</span>
                {r.pitch_count > 0 && <span style={{background:'#dcfce7',color:'#166534',borderRadius:20,padding:'2px 10px',fontSize:12,fontWeight:600}}>{r.pitch_count} pitch{r.pitch_count!==1?'es':''}</span>}
              </div>
            </div>
            <button onClick={() => del(r.id)} disabled={deleting===r.id}
              style={{padding:'6px 12px',background:'#fee2e2',color:'#dc2626',border:'none',borderRadius:6,cursor:'pointer',fontSize:12,fontWeight:600,whiteSpace:'nowrap'}}>
...
// ── PITCHES TAB ──────────────────────────────────────────────────────────────
function PitchesTab({ token, user, notify }) {
  const [pitches, setPitches] = useState([]);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    api('/pitches?mine=1', {}, token)
      .then(d => setPitches(d.pitches || d || []))
      .catch(() => {})
      .finally(() => setLoading(false));
  }, [token]);
  if (loading) return <div style={{padding:40,textAlign:'center'}}><Spin /></div>;
  if (!pitches.length) return (
    <div style={{textAlign:'center',padding:40,color:'#6b7280'}}>
      <div style={{fontSize:40,marginBottom:12}}>💬</div>
      <div style={{fontWeight:600,marginBottom:6}}>No pitches yet</div>
      <div style={{fontSize:14}}>Browse buyer requests and pitch your items.</div>
    </div>
  );
  return (
    <div style={{display:'flex',flexDirection:'column',gap:12}}>
      {pitches.map(p => (
        <div key={p.id} style={{background:'white',borderRadius:10,padding:16,boxShadow:'0 1px 3px rgba(0,0,0,.07)'}}>
          <div style={{fontWeight:600,fontSize:15,marginBottom:4}}>{p.request_title || 'Buyer Request'}</div>
          <div style={{fontSize:13,color:'#6b7280',marginBottom:8}}>{p.message?.slice(0,120)}{p.message?.length>120?'…':''}</div>
          <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
            {p.price && <span style={{background:'#dbeafe',color:'#1e40af',borderRadius:20,padding:'2px 10px',fontSize:12}}>Offered: {fmtKES(p.price)}</span>}
            <span style={{background:p.status==='accepted'?'#dcfce7':p.status==='rejected'?'#fee2e2':'#f3f4f6',
              color:p.status==='accepted'?'#166534':p.status==='rejected'?'#dc2626':'#374151',
              borderRadius:20,padding:'2px 10px',fontSize:12,fontWeight:600}}>
              {p.status==='accepted'?'✓ Accepted':p.status==='rejected'?'✗ Declined':'Pending'}
            </span>
            <span style={{fontSize:12,color:'#9ca3af'}}>{ago(p.created_at)}</span>
          </div>
        </div>
      ))}
    </div>
...
// ── PROFILE SECTION ──────────────────────────────────────────────────────────
function ProfileSection({ user, token, notify, onUserUpdate }) {
  const [form, setForm] = useState({ name: user?.name||'', phone: user?.phone||'', county: user?.county||'', bio: user?.bio||'' });
  const [saving, setSaving] = useState(false);
  ...
  const save = async e => {
    ...
    try {
      ...
      const updated = await api('/auth/me', { method: 'PATCH', body }, token);
      onUserUpdate && onUserUpdate(updated.user || updated);
      notify('Profile updated!', 'success');
    } catch (e) { notify(e.message, 'error'); }
    finally { setSaving(false); }
  };
  return (
    <form onSubmit={save} style={{maxWidth:500}}>
      <div style={{marginBottom:20,textAlign:'center'}}>
        <div style={{width:80,height:80,borderRadius:'50%',background:'#e5e7eb',margin:'0 auto 12px',overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'center'}}>
          {(user?.photo||photo) ? <img src={photo?URL.createObjectURL(photo):user.photo} style={{width:'100%',height:'100%',objectFit:'cover'}}/> : <span style={{fontSize:32}}>👤</span>}
        </div>
        <label style={{cursor:'pointer',color:'#2563eb',fontSize:13,fontWeight:600}}>
          Change Photo
          <input type="file" accept="image/*" style={{display:'none'}} onChange={e=>setPhoto(e.target.files[0])}/>
        </label>
      </div>
      {[['name','Full Name','text'],['phone','Phone Number','tel'],['bio','Bio','text']].map(([k,label,type])=>(
        <div key={k} style={{marginBottom:16}}>
          <label style={{display:'block',marginBottom:6,fontWeight:600,fontSize:14,color:'#374151'}}>{label}</label>
          {k==='bio'
            ? <textarea value={form[k]} onChange={e=>setForm(f=>({...f,[k]:e.target.value}))} rows={3}
                style={{width:'100%',padding:'10px 12px',border:'1px solid #e5e7eb',borderRadius:8,fontSize:14,resize:'vertical',fontFamily:'inherit'}}/>
            : <input type={type} value={form[k]} onChange={e=>setForm(f=>({...f,[k]:e.target.value}))}
                style={{width:'100%',padding:'10px 12px',border:'1px solid #e5e7eb',borderRadius:8,fontSize:14}}/>}
        </div>
      ))}
      <div style={{marginBottom:16}}>
        <label style={{display:'block',marginBottom:6,fontWeight:600,fontSize:14,color:'#374151'}}>County</label>
      ...
      <button type="submit" disabled={saving}
        style={{width:'100%',padding:'12px',background:'#2563eb',color:'white',border:'none',borderRadius:8,fontWeight:600,fontSize:15,cursor:'pointer'}}>
...
// ── PASSWORD SECTION ─────────────────────────────────────────────────────────
function PasswordSection({ token, notify }) {
  const [form, setForm] = useState({ current: '', next: '', confirm: '' });
  const [saving, setSaving] = useState(false);
  const save = async e => {
    e.preventDefault();
    if (form.next !== form.confirm) return notify('Passwords do not match', 'error');
    if (form.next.length < 8) return notify('Password must be at least 8 characters', 'error');
    setSaving(true);
    try {
      await api('/auth/change-password', { method: 'POST', body: JSON.stringify({ currentPassword: form.current, newPassword: form.next }) }, token);
      notify('Password changed successfully!', 'success');
  ...
  return (
    <form onSubmit={save} style={{maxWidth:400}}>
      {[['current','Current Password'],['next','New Password'],['confirm','Confirm New Password']].map(([k,label])=>(
        <div key={k} style={{marginBottom:16}}>
          <label style={{display:'block',marginBottom:6,fontWeight:600,fontSize:14,color:'#374151'}}>{label}</label>
          <PasswordField value={form[k]} onChange={v=>setForm(f=>({...f,[k]:v}))} placeholder={label}/>
        </div>
      ))}
      <button type="submit" disabled={saving}
        style={{width:'100%',padding:'12px',background:'#2563eb',color:'white',border:'none',borderRadius:8,fontWeight:600,fontSize:15,cursor:'pointer'}}>
...
// ── VERIFICATION SECTION ─────────────────────────────────────────────────────
function VerificationSection({ user, token, notify, onUserUpdate }) {
  const [file, setFile] = useState(null);
  const [uploading, setUploading] = useState(false);
  if (user?.is_verified) return (
    <div style={{textAlign:'center',padding:40}}>
      <div style={{fontSize:60,marginBottom:12}}>✅</div>
      <div style={{fontWeight:700,fontSize:20,color:'#166534',marginBottom:8}}>Identity Verified</div>
      <div style={{color:'#6b7280'}}>Your account is verified. Buyers can trust your listings.</div>
    </div>
  );
  const submit = async e => {
    e.preventDefault();
    if (!file) return notify('Please select your ID photo', 'error');
    setUploading(true);
    try {
      const body = new FormData();
      body.append('id_photo', file);
      const r = await api('/auth/verify-id', { method: 'POST', body }, token);
      notify('ID submitted for review! You\'ll be notified once verified.', 'success');
      onUserUpdate && onUserUpdate({ ...user, verification_pending: true });
    } catch (e) { notify(e.message, 'error'); }
    finally { setUploading(false); }
  };
  return (
    <div style={{maxWidth:480}}>
      <div style={{background:'#fefce8',border:'1px solid #fde68a',borderRadius:10,padding:16,marginBottom:24}}>
        <div style={{fontWeight:600,marginBottom:4}}>📋 Why verify?</div>
        <ul style={{margin:0,paddingLeft:20,color:'#92400e',fontSize:14,lineHeight:1.8}}>
          <li>Get a verified badge on all your listings</li>
          <li>Build trust with buyers</li>
          <li>Required for escrow transactions above KSh 10,000</li>
        </ul>
      </div>
      {user?.verification_pending
        ? <div style={{textAlign:'center',padding:30,color:'#6b7280'}}><div style={{fontSize:48,marginBottom:12}}>⏳</div><div style={{fontWeight:600}}>Verification Pending</div><div style={{fontSize:14,marginTop:8}}>We're reviewing your ID. This usually takes 1-2 business days.</div></div>
        : <form onSubmit={submit}>
            <div style={{marginBottom:16}}>
              <label style={{display:'block',marginBottom:6,fontWeight:600,fontSize:14}}>Upload National ID / Passport</label>
              <input type="file" accept="image/*,.pdf" onChange={e=>setFile(e.target.files[0])}
                style={{width:'100%',padding:'10px',border:'2px dashed #e5e7eb',borderRadius:8,cursor:'pointer'}}/>
              {file && <div style={{marginTop:8,fontSize:13,color:'#6b7280'}}>Selected: {file.name}</div>}
            </div>
            <p style={{fontSize:13,color:'#6b7280',marginBottom:16}}>Your ID is only used for verification and will not be publicly displayed.</p>
            <button type="submit" disabled={uploading||!file}
              style={{width:'100%',padding:'12px',background:file?'#2563eb':'#e5e7eb',color:file?'white':'#9ca3af',border:'none',borderRadius:8,fontWeight:600,fontSize:15,cursor:file?'pointer':'default'}}>
...
function Dashboard({ user, token, notify, onPostAd, onClose, onUserUpdate, initialTab }) {
  ...
  const [stats, setStats] = useState({ active: 0, sold: 0, views: 0 });
  const loadListings = () => {
    setLoadingL(true);
    api('/listings?mine=1&perPage=50', {}, token)
      .then(d => { setListings(d.listings||[]); setStats({active:(d.listings||[]).filter(l=>l.status==='active').length,sold:(d.listings||[]).filter(l=>l.status==='sold').length,views:(d.listings||[]).reduce((a,l)=>a+(l.views||0),0)}); })
      .catch(()=>{})
  ...
  const delListing = async id => {
    ...
    try { await api('/listings/'+id, {method:'DELETE'}, token); notify('Listing deleted'); loadListings(); }
    catch(e) { notify(e.message,'error'); }
  };
  return (
    <div style={{minHeight:'100vh',background:'#f8fafc'}}>
      <div style={{background:'white',borderBottom:'1px solid #e5e7eb',padding:'16px 24px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          ...
          <div>
            <div style={{fontWeight:700,fontSize:18}}>My Dashboard</div>
            <div style={{fontSize:13,color:'#6b7280'}}>{user?.name || user?.email}</div>
          </div>
        </div>
        <button onClick={onPostAd} style={{background:'#2563eb',color:'white',border:'none',borderRadius:8,padding:'8px 16px',fontWeight:600,cursor:'pointer',fontSize:14}}>
          + Post Ad
        </button>
      </div>
      <div style={{maxWidth:1000,margin:'0 auto',padding:'24px 16px'}}>
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginBottom:24}}>
          {[['Active Listings',stats.active,'#dbeafe','#1e40af'],['Sold',stats.sold,'#dcfce7','#166534'],['Total Views',stats.views,'#fef3c7','#92400e']].map(([l,v,bg,col])=>(
            <div key={l} style={{background:bg,borderRadius:10,padding:'16px',textAlign:'center'}}>
              ...
              <div style={{fontSize:13,color:col,marginTop:4}}>{l}</div>
            </div>
          ))}
        </div>
        <div style={{display:'flex',gap:4,marginBottom:24,flexWrap:'wrap',background:'white',borderRadius:10,padding:8,boxShadow:'0 1px 3px rgba(0,0,0,.08)'}}>
          {TABS.map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)}
              style={{padding:'8px 14px',border:'none',borderRadius:8,cursor:'pointer',fontSize:13,fontWeight:tab===t.id?700:400,
                background:tab===t.id?'#2563eb':'transparent',color:tab===t.id?'white':'#374151',transition:'all .15s'}}>
              {t.label}
            </button>
          ))}
        </div>
        <div style={{background:'white',borderRadius:10,padding:20,boxShadow:'0 1px 3px rgba(0,0,0,.08)'}}>
          {tab==='listings' && (
            loadingL ? <div style={{textAlign:'center',padding:40}}><Spin /></div> :
            !listings.length ? (
              <div style={{textAlign:'center',padding:40,color:'#6b7280'}}>
                <div style={{fontSize:48,marginBottom:12}}>📦</div>
                <div style={{fontWeight:600,marginBottom:8}}>No listings yet</div>
                <button onClick={onPostAd} style={{background:'#2563eb',color:'white',border:'none',borderRadius:8,padding:'10px 24px',fontWeight:600,cursor:'pointer'}}>Post Your First Ad</button>
              </div>
            ) : (
              <div>
                {listings.map(l=>(
                  <div key={l.id} style={{display:'flex',gap:12,padding:'12px 0',borderBottom:'1px solid #f3f4f6',alignItems:'center'}}>
                    {l.photos?.[0] && <img src={l.photos[0]} style={{width:72,height:72,borderRadius:8,objectFit:'cover',flexShrink:0}}/>}
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontWeight:600,fontSize:15,marginBottom:2,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{l.title}</div>
                      <div style={{color:'#2563eb',fontWeight:700,marginBottom:4}}>{fmtKES(l.price)}</div>
                      <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                        <span style={{background:l.status==='active'?'#dcfce7':l.status==='sold'?'#e0e7ff':'#f3f4f6',color:l.status==='active'?'#166534':l.status==='sold'?'#4338ca':'#6b7280',borderRadius:20,padding:'2px 8px',fontSize:11,fontWeight:600}}>{l.status}</span>
                        <span style={{fontSize:12,color:'#9ca3af'}}>{l.views||0} views</span>
                        <span style={{fontSize:12,color:'#9ca3af'}}>{ago(l.created_at)}</span>
                      </div>
                    </div>
                    <button onClick={()=>delListing(l.id)} style={{padding:'6px 12px',background:'#fee2e2',color:'#dc2626',border:'none',borderRadius:6,cursor:'pointer',fontSize:12,fontWeight:600,flexShrink:0}}>Delete</button>
                  </div>
                ))}
              </div>
            )
          )}
          {tab==='requests' && <MyRequestsTab token={token} user={user} notify={notify}/>}
          {tab==='pitches' && <PitchesTab token={token} user={user} notify={notify}/>}
          {tab==='reviews' && <ReviewsSection token={token} userId={user?.id} isSelf={true}/>}
          {tab==='profile' && <ProfileSection user={user} token={token} notify={notify} onUserUpdate={onUserUpdate}/>}
          {tab==='password' && <PasswordSection token={token} notify={notify}/>}
          {tab==='verify' && <VerificationSection user={user} token={token} notify={notify} onUserUpdate={onUserUpdate}/>}
        </div>
      </div>
    </div>
  );
...
// ── MOBILE REQUESTS TAB ──────────────────────────────────────────────────────
function MobileRequestsTab({ token, user, notify, onClose }) {
  const [tab, setTab] = useState('browse');
  return (
    <div style={{minHeight:'100vh',background:'#f8fafc'}}>
      <div style={{background:'white',padding:'14px 16px',display:'flex',alignItems:'center',gap:12,borderBottom:'1px solid #e5e7eb'}}>
        <button onClick={onClose} style={{background:'none',border:'none',fontSize:20,cursor:'pointer'}}>←</button>
        <div style={{fontWeight:700,fontSize:17}}>Requests</div>
      </div>
      <div style={{display:'flex',background:'white',borderBottom:'1px solid #e5e7eb'}}>
        {[['browse','Browse'],['mine','My Requests'],['pitches','My Pitches']].map(([id,label])=>(
          <button key={id} onClick={()=>setTab(id)}
            style={{flex:1,padding:'12px 4px',border:'none',background:'none',cursor:'pointer',fontSize:13,fontWeight:tab===id?700:400,
              color:tab===id?'#2563eb':'#6b7280',borderBottom:tab===id?'2px solid #2563eb':'2px solid transparent'}}>
            {label}
          </button>
        ))}
      </div>
      <div style={{padding:16}}>
        {tab==='browse' && <WhatBuyersWant token={token} user={user} notify={notify} compact={false}/>}
        {tab==='mine' && (user ? <MyRequestsTab token={token} user={user} notify={notify}/> : <div style={{textAlign:'center',padding:40,color:'#6b7280'}}>Sign in to see your requests</div>)}
        {tab==='pitches' && (user ? <PitchesTab token={token} user={user} notify={notify}/> : <div style={{textAlign:'center',padding:40,color:'#6b7280'}}>Sign in to see your pitches</div>)}
      </div>
    </div>
  );
}
...
// ── MOBILE DASHBOARD ─────────────────────────────────────────────────────────
function MobileDashboard({ user, token, notify, onPostAd, onClose, onUserUpdate }) {
  ...
  const TABS = [['listings','My Ads'],['requests','Requests'],['reviews','Reviews'],['profile','Profile']];
  return (
    <div style={{minHeight:'100vh',background:'#f8fafc'}}>
      <div style={{background:'white',padding:'14px 16px',display:'flex',alignItems:'center',justifyContent:'space-between',borderBottom:'1px solid #e5e7eb'}}>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          <button onClick={onClose} style={{background:'none',border:'none',fontSize:20,cursor:'pointer'}}>←</button>
          <div style={{fontWeight:700,fontSize:17}}>Dashboard</div>
        </div>
        <button onClick={onPostAd} style={{background:'#2563eb',color:'white',border:'none',borderRadius:8,padding:'7px 14px',fontWeight:600,cursor:'pointer',fontSize:13}}>+ Post Ad</button>
      </div>
      <div style={{display:'flex',overflowX:'auto',background:'white',borderBottom:'1px solid #e5e7eb',padding:'0 8px'}}>
        {TABS.map(([id,label])=>(
          <button key={id} onClick={()=>setTab(id)}
            style={{padding:'12px 16px',border:'none',background:'none',cursor:'pointer',fontSize:13,fontWeight:tab===id?700:400,
              color:tab===id?'#2563eb':'#6b7280',borderBottom:tab===id?'2px solid #2563eb':'2px solid transparent',whiteSpace:'nowrap'}}>
            {label}
          </button>
        ))}
      </div>
      <div style={{padding:16}}>
        {tab==='listings' && (
          ...
          !listings.length
            ? <div style={{textAlign:'center',padding:40,color:'#6b7280'}}>
                <div style={{fontSize:40,marginBottom:12}}>📦</div>
                <div style={{fontWeight:600,marginBottom:12}}>No listings yet</div>
                <button onClick={onPostAd} style={{background:'#2563eb',color:'white',border:'none',borderRadius:8,padding:'10px 24px',fontWeight:600,cursor:'pointer'}}>Post Your First Ad</button>
              </div>
            : listings.map(l=>(
              <div key={l.id} style={{background:'white',borderRadius:10,padding:14,marginBottom:12,boxShadow:'0 1px 3px rgba(0,0,0,.07)',display:'flex',gap:12,alignItems:'center'}}>
                {l.photos?.[0] && <img src={l.photos[0]} style={{width:64,height:64,borderRadius:8,objectFit:'cover',flexShrink:0}}/>}
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontWeight:600,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',fontSize:14}}>{l.title}</div>
                  <div style={{color:'#2563eb',fontWeight:700,fontSize:14}}>{fmtKES(l.price)}</div>
                  <div style={{fontSize:12,color:'#9ca3af'}}>{l.status} · {l.views||0} views</div>
                </div>
              </div>
            ))
        )}
...
// ── MOBILE LAYOUT ────────────────────────────────────────────────────────────
function MobileLayout({ user, token, notify, page, setPage, listings, total, loading, filter, setFilter, pg, setPg, stats, counties, modal, setModal, notifCount, mobileFiltersOpen, setMobileFiltersOpen, mobileTab, setMobileTab, openListing, handleLockIn }) {
  const [showDash, setShowDash] = useState(false);
  const [showRequests, setShowRequests] = useState(false);
  const [menuOpen, setMenuOpen] = useState(false);
  if (showDash && user) return (
    <MobileDashboard user={user} token={token} notify={notify}
      onPostAd={()=>{setShowDash(false);setModal({type:'post'});}}
      onClose={()=>setShowDash(false)}
      onUserUpdate={u=>{}}/>
  );
  if (showRequests) return (
    <MobileRequestsTab token={token} user={user} notify={notify} onClose={()=>setShowRequests(false)}/>
  );
  const NAV = [
  ...
  return (
    <div style={{paddingBottom:70,minHeight:'100vh',background:'#f8fafc'}}>
      {/* Header */}
      <div style={{background:'white',padding:'12px 16px',display:'flex',alignItems:'center',justifyContent:'space-between',borderBottom:'1px solid #e5e7eb',position:'sticky',top:0,zIndex:100}}>
        <WekaSokoLogo size={28}/>
        <div style={{display:'flex',gap:8,alignItems:'center'}}>
          <button onClick={()=>setMobileFiltersOpen(v=>!v)} style={{background:'#f3f4f6',border:'none',borderRadius:8,padding:'7px 12px',cursor:'pointer',fontSize:13,fontWeight:600}}>
            🔍 Filter
          </button>
          {user && <button onClick={()=>setShowDash(true)} style={{background:'none',border:'none',cursor:'pointer',fontSize:22}}>👤</button>}
          {!user && <button onClick={()=>setModal({type:'auth'})} style={{background:'#2563eb',color:'white',border:'none',borderRadius:8,padding:'7px 14px',cursor:'pointer',fontSize:13,fontWeight:600}}>Sign In</button>}
        </div>
      </div>
      {/* Filters panel */}
      {mobileFiltersOpen && (
        <div style={{background:'white',padding:16,borderBottom:'1px solid #e5e7eb'}}>
          <input value={filter.q||''} onChange={e=>setFilter(f=>({...f,q:e.target.value,page:1}))} placeholder="Search listings…"
          ...
          <div style={{display:'flex',gap:8}}>
            <select value={filter.county||''} onChange={e=>setFilter(f=>({...f,county:e.target.value,page:1}))}
              style={{flex:1,padding:'9px 8px',border:'1px solid #e5e7eb',borderRadius:8,fontSize:13}}>
              <option value="">All Counties</option>
              {KENYA_COUNTIES.map(c=><option key={c} value={c}>{c}</option>)}
            </select>
            <select value={filter.sort||'newest'} onChange={e=>setFilter(f=>({...f,sort:e.target.value,page:1}))}
              style={{flex:1,padding:'9px 8px',border:'1px solid #e5e7eb',borderRadius:8,fontSize:13}}>
      ...
      <div style={{padding:'12px 16px'}}>
        {mobileTab==='requests'
          ? <WhatBuyersWant token={token} user={user} notify={notify} compact={false}/>
          : loading
            ? <div style={{textAlign:'center',padding:60}}><Spin /></div>
            : !listings.length
              ? <div style={{textAlign:'center',padding:60,color:'#6b7280'}}><div style={{fontSize:40,marginBottom:12}}>🔍</div><div>No listings found</div></div>
              : <>
                  <div style={{fontSize:13,color:'#6b7280',marginBottom:12}}>{total} listing{total!==1?'s':''} found</div>
                  <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:10}}>
                    {listings.map(l=><ListingCard key={l.id} listing={l} onClick={()=>openListing(l)} onLockIn={()=>handleLockIn(l)} token={token} user={user}/>)}
                  </div>
                  <Pager total={total} perPage={PER_PAGE} page={pg} onChange={p=>{setPg(p);window.scrollTo({top:0,behavior:'smooth'});}}/>
                </>
        }
      </div>
      {/* Bottom Nav */}
      <div style={{position:'fixed',bottom:0,left:0,right:0,background:'white',borderTop:'1px solid #e5e7eb',display:'flex',zIndex:200,boxShadow:'0 -2px 10px rgba(0,0,0,.07)'}}>
        {NAV.map(n=>(
          <button key={n.id} onClick={()=>{
            if(n.id==='post'){setModal({type:'post'});return;}
            if(n.id==='account'){user?setShowDash(true):setModal({type:'auth'});return;}
            if(n.id==='requests'){setShowRequests(true);return;}
            if(n.id==='chat'&&!user){setModal({type:'auth'});return;}
            setMobileTab(n.id==='home'?'listings':n.id);
          }}
            ...
            <span style={{fontSize:20,position:'relative'}}>
              {n.icon}
              {n.id==='chat'&&notifCount>0&&<span style={{position:'absolute',top:-4,right:-4,background:'#ef4444',color:'white',borderRadius:99,fontSize:9,padding:'1px 4px',fontWeight:700,minWidth:14,textAlign:'center'}}>{notifCount}</span>}
            </span>
            <span style={{fontSize:10,color:'#6b7280',fontWeight:500}}>{n.label}</span>
          </button>
        ))}
      </div>
    </div>
...
// ── EXPORTS ──────────────────────────────────────────────────────────────────
export {
  WekaSokoLogo, Spin, Toast, Modal, FF, Counter, ImageUploader, TermsModal,
  PasswordField, ForgotPasswordPanel, ResetPasswordModal, WatermarkedImage,
  Lightbox, AuthModal, ShareModal, PayModal, ChatModal, PostAdModal,
  ListingCard, DetailModal, LeaveReviewBtn, ReportListingBtn, VerificationBanner,
  MarkSoldModal, RoleSwitcher, PostRequestModal, WhatBuyersWant,
  Pager, PWABanner, SoldSection, StarPicker, ReviewsSection,
  MyRequestsTab, PitchesTab, ProfileSection, PasswordSection,
  VerificationSection, MobileDashboard, Dashboard,
  MobileRequestsTab, MobileLayout
};
